# 纠纷材料整理组件 6.5.3 / Skill 6.5.1 发布步骤

1. 应用和飞书用户身份必须已开通并发布 `drive:drive`、`drive:file:upload`、`minutes:minutes.upload:write`、`minutes:minutes.basic:read`、`minutes:minutes.artifacts:read` 和 `docs:permission.member:create`；
2. 保留案件表隐藏文本字段 `材料处理基线`；
3. 新建或更新子智能体“纠纷材料视觉核验员”，模型固定 `Doubao-Seed-2.1-turbo`，使用 `视觉核验子智能体员工档案配置.md`，不给予 Base、文档或权限写入工具；
4. 导入 `organize-dispute-materials-v6.5.1.zip`；
5. 主智能体模型固定 `Deepseek-V4-Pro`，添加上述子智能体，并用 `智能体员工档案配置.md` 更新员工档案；
6. 发布小组件 `6.5.3-supplement-link`；
